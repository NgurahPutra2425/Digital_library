...
$id = $_GET['id'];
$data = mysqli_query($koneksi, "SELECT * FROM ulasanbuku LEFT JOIN user ON ulasanbuku.UserID = user.UserID LEFT JOIN buku ON ulasanbuku.BukuID = buku.BukuID WHERE UlasanID = '$id'");
...
<div class="flex w-full h-screen items-center justify-center text-left capitalize">
    <form action="./proses-update.php" class="flex flex-col relative justify-center items-center border border-[3px] border-black rounded-xl h-[60%] w-[40%] z-99" method="post">
        ...
        <select name="buku" id="buku" class="bg-blue-300 w-[95%] placeholder:px-2 rounded-lg mb-3" disabled>
            ...
            ?>
                <option <?php if ($d['BukuID'] == $row['BukuID']) {
                        ...
                        ?> value="<?php echo $d['BukuID'] ?>" class="w-[100%] bg-cyan-400 rounded-lg my-2">
                    <?php echo $d['Judul'] ?>
                </option>
            <?php
            }
        ...
        <input type="text" class="bg-blue-300 w-[95%] placeholder:px-2 rounded-lg mb-3" value="<?php echo $row['Ulasan'] ?>" name="ulasan" id="ulasan" required>
        <label for="rating" class="w-[95%] text-lg font-bold">Rating : </label>
        <input type="number" class="bg-blue-300 w-[95%] placeholder:px-2 rounded-lg mb-3" value="<?php echo $row['Rating'] ?>" min="1" max="10" placeholder="-/10" name="rating" id="rating" required>
